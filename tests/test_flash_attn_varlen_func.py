...
@pytest.mark.flash_attn_varlen_func
def test_flash_attn_varlen_fa3_stable_autotune_selection() -> None:
    scheduling = _fa3_scheduling_module()
    heuristics = scheduling.PersistentSchedulingHeuristics
    ...
    noisy_tie = {
        noisy: (1.00, 0.99, 1.15),
        stable: (1.02, 1.01, 1.03),
    }
    assert heuristics.select_stable_config(noisy_tie) is stable

    clear_noisy_win = {
        noisy: (0.95, 0.94, 0.96),
        stable: (1.02, 1.01, 1.03),
    }
    assert heuristics.select_stable_config(clear_noisy_win) is noisy


@pytest.mark.flash_attn_varlen_func
def test_flash_attn_varlen_fa3_standard_block_n_heuristic(
    monkeypatch,
) -> None:
    scheduling = _fa3_scheduling_module()
    heuristics = scheduling.PersistentSchedulingHeuristics
    ...
    for head_dim, expected_block_n in (
        (64, {64, 128}),
        (128, {64, 128}),
        (192, {64}),
        (256, {64}),
    ...
    for config in heuristics.autotune_configs():
        assert config.kwargs["BLOCK_N"] in (64, 128)
        assert set(config.kwargs) == {
            "BLOCK_M",
            "BLOCK_N",
            "NUM_BUFFERS_Q",
            "NUM_BUFFERS_KV",
    ...
    assert config.kwargs["BLOCK_N"] == 128
...
@pytest.mark.flash_attn_varlen_func
def test_flash_attn_varlen_fa3_n64_paged_prefill_policy_matrix() -> None:
    scheduling = _fa3_scheduling_module()
    heuristics = scheduling.PersistentSchedulingHeuristics
    configs = heuristics.autotune_configs()
    base = {
    ...
    kept = heuristics.prune_autotune_configs(configs, {}, **base)
    assert all(config.kwargs["BLOCK_N"] == 64 for config in kept)
    assert any(config.kwargs["STAGGER_KV"] for config in kept)
    ...
    for override in negative_overrides:
        ...
        assert all(config.kwargs["BLOCK_N"] in (64, 128) for config in kept), override
        if case["is_paged"] and case["PAGED_KV_NON_TMA"]:
            page_size = case["block_size"]
            assert all(
                config.kwargs["BLOCK_N"] % page_size == 0
                for config in kept
            ), override
