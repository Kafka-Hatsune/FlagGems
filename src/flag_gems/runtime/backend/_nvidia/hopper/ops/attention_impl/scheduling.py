...
class PersistentSchedulingHeuristics:
    ...
    # Version 18 removes the non-standard active-N tuning dimension. Persisted
    # rows now describe only the physical BLOCK_N used by the standard WGMMA
    ...
    def select_stable_config(cls, timings):
        """Select the candidate with the best spread-adjusted latency."""
        ...
        return min(tuple(timings), key=lambda config: scores(config)[1])
    ...
    def prune_autotune_configs(cls, configs, nargs, **kwargs):
        ...
        def page_extent_compatible(config):
            return (
                ...
                or config.kwargs["BLOCK_N"] % block_size == 0
            )

        def smem_compatible(config):
            return cls.config_smem_bytes(config, head_dim) <= 220 * 1024

        def selected_shape_config(config):
            stagger_kv = config.kwargs.get("STAGGER_KV", False)
            if stagger_kv and not wide_paged_prefill_packgqa_ws:
                return False
        ...
        if is_paged and paged_kv_non_tma:
            raise ValueError(
                "paged non-TMA has no legal BLOCK_N: it must be "
