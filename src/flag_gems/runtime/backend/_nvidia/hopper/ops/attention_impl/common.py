...
def _paged_tile_cache_state(
    ...
    """Load one reusable physical base for every page covered by an N tile."""
    ...
    page_stride_rows = page_stride_rows.to(tl.int64)
    page_bases = tl.tuple(())
    NUM_PAGES: tl.constexpr = BLOCK_N // block_size
    first_page = n_start // block_size
    for page_entry in tl.static_range(0, NUM_PAGES):
        ...
        page_bases += (page_block * page_stride_rows,)
    return page_bases
...
def _make_paged_kv_descriptor(
    ...
    """Create a standard power-of-two paged KV descriptor tile."""
...
def _make_dense_kv_descriptor(
    ...
    """Create a standard power-of-two dense KV descriptor tile."""
...
def _copy_paged_kv_tile_to_pipe(
    ...
    page = rows // block_size
    row_in_page = rows % block_size

    cache_idx = cache_state[0] + row_in_page
    for page_idx in tl.static_range(1, NUM_PAGES):
        cache_idx = tl.where(
            page == page_idx,
            cache_state[page_idx] + row_in_page,
            cache_idx,
        )
